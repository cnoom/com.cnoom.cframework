    ...
    public class AddressableManagerWindow : EditorWindow
    {
        private void OnGUI()
        {
            ...
            for (var i = 0; i < settings.markedFolderGuids.Count; i++)
            {
                ...
                var path = AssetDatabase.GUIDToAssetPath(settings.markedFolderGuids[i]);
            ...
            // 操作按钮
            if (GUILayout.Button("将可寻址文件夹内部内容标记为可循址资源")) AddressableProcessor.ProcessAllMarkedFolders();
