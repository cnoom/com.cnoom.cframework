...
namespace CnoomFrameWork.Core
{
    ...
    public class ModuleManager
    {
        ...
        {
            var module = InstanceFactory.CreateInstance<TModule>(_rootContainer);
        ...
        {
            var module = _rootContainer.Resolve<TInterface>();
        ...
        {
            var config = ConfigManager.Instance.GetConfig<ModuleOrderConfig>();
            foreach (var handler in config.Registers)
            {
                var module = handler.Register(_rootContainer) as Module;
                EventManager.Register(module);
                module.Initialize();
            }
        }
