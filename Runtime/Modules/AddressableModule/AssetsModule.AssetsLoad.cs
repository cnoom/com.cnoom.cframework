...
namespace CnoomFrameWork.Modules.AddressableModule
{
    public partial class AssetsModule
    {
        ...
        public T LoadAsset<T>(string key)
        {
            ...
            {
                Log.ColorLogErrorEx(nameof(AssetsModule), "资源键无效!");
                return default;
            }
            try
            {
                ...
                {
                    Log.ColorLogErrorEx(nameof(AssetsModule), $"加载失败: 无法找到地址键 {key}");
                    return default;
                }
            ...
            {
                Log.ColorLogErrorEx(nameof(AssetsModule), $"加载失败: {e.Message}");
                return default;
            }
        }
        ...
        public IEnumerator LoadAssetAsyncRoutine<T>(string key, Action<T> onLoaded)
        {
            ...
            {
                Log.ColorLogErrorEx(nameof(AssetsModule), $"加载失败: {key} [{operation.OperationException}]");
        ...
        public void LoadByReference<T>(AssetReference reference, Action<T> onLoaded)
        {
            ...
            {
                Log.ColorLogErrorEx(nameof(AssetsModule), "无效的AssetReference");
                return;
            }
        ...
        public IEnumerator LoadAssetByReferenceCoroutine<T>(AssetReference reference, Action<T> onLoaded)
        {
            ...
            if(operation.Status != AsyncOperationStatus.Succeeded)
            {
                Log.ColorLogErrorEx(nameof(AssetsModule), $"加载失败: {reference.AssetGUID} [{operation.OperationException}]");
                yield break;
            }
            onLoaded?.Invoke(operation.Result);
        }
        ...
        public IEnumerator LoadAssetsByLabelCoroutine<T>(string label, Action<T> onLoaded, Action<float> onProgress = null)
        {
            ...
            for(var i = 0; i < locations.Count; i++)
            {
                ...
                if(operation.Status != AsyncOperationStatus.Succeeded)
                {
                    Log.ColorLogErrorEx(nameof(AssetsModule), $"加载失败: {location.PrimaryKey} [{operation.OperationException}]");
                    continue;
                }
