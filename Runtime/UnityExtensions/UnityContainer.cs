...
namespace CnoomFrameWork.Core.UnityExtensions
{
    ...
    {
        private readonly Dictionary<string, GameObjectContainer> _gameObjects = new();
        ...
        public void Dispose()
        {
            foreach (var gameObjectContainer in _gameObjects.Values) gameObjectContainer.Dispose();
        }

        public T GetComponent<T>(string name) where T : Component
        {
            return _gameObjects[name].GetComponent<T>();
        }

        public Transform GetTransform(string name)
        {
            return _gameObjects[name].Transform;
        }

        public GameObject GetGameObject(string name)
        {
            return _gameObjects[name].GameObject;
        }

        public void AddGameObject(GameObject gameObject)
        {
            _gameObjects.Add(gameObject.name, new GameObjectContainer
            {
        ...
        {
            _gameObjects.Add(name, new GameObjectContainer
            {
                GameObject = gameObject
            });
        }
        ...
        {
            if (!_gameObjects.TryGetValue(name, out var gameObjectContainer)) return false;
        ...
        private class GameObjectContainer : IDisposable
        {
            ...
            public T GetComponent<T>() where T : Component
            {
                ...
                var tComponent = GameObject.GetComponent<T>();
